<?php 
/**
 * @version $Id: default.php 40 2014-09-08 14:28:34Z szymon $
...
<form action="<?php echo JRoute::_('index.php?option=com_djmediatools&view=items'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th width="8%">
					<?php echo JText::_('COM_DJMEDIATOOLS_IMAGE'); ?>
				</th>
				<th>
				...
				<th width="10%">
					<?php echo JHtml::_('grid.sort', 'COM_DJMEDIATOOLS_CATEGORY', 'category_title', $listDirn, $listOrder); ?>
				</th>
				<th width="1%">
		...
		foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td align="center">
					<?php if ($item->video) : ?>
						<a class="modal item-thumb" rel="{handler: 'iframe', size: {x: 850, y: 510} }" href="<?php echo $item->video; ?>?autoplay=1"><img src="<?php echo $item->thumb; ?>" alt="<?php echo $this->escape($item->title); ?>" style="border: 1px solid #ccc; padding: 1px;" /><span class="video-icon"></span></a>
					<?php elseif ($item->image) : ?>
						<a class="modal item-thumb" href="<?php echo $item->image; ?>"><img src="<?php echo $item->thumb; ?>" alt="<?php echo $this->escape($item->title); ?>" style="border: 1px solid #ccc; padding: 1px;" /></a>
					<?php endif; ?>
				</td>
				<td>
					...
					<?php if ($canEdit || $canEditOwn) : ?>
						<a href="<?php echo JRoute::_('index.php?option=com_djmediatools&task=item.edit&id='.(int) $item->id); ?>">
							<?php echo $this->escape($item->title); ?></a>
					<?php else : ?>
						<?php echo $this->escape($item->title); ?>
					<?php endif; ?>
					<div class="small">
						<?php echo $this->escape($item->alias); ?></div>
					<div class="small">
						<?php 
